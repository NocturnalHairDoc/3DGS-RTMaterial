...
extern "C" __global__ void __raygen__rg() {
           ...
           (getNextTraceState() != PGRNDTraceTerminate))
    {
        ...
        // -- Per-segment accumulation: each path segment contributes independently.
        // pathThroughput carries all prior attenuation, so no cross-bounce bookkeeping needed.

        // Aggregate volumetric radiance for this segment
        payload.accumulatedAlpha += density * (1.0f - payload.accumulatedAlpha);
        payload.accumulatedColor += payload.pathThroughput * radiance;

        // Volumetric transmittance attenuates throughput for the remainder of the path
        payload.pathThroughput *= (1.0f - density);

        // Surface emission at this hit point (only this segment's emissive)
        payload.accumulatedColor += payload.pathThroughput * payload.nextEmissive;

        // BSDF/BTDF scatter attenuates throughput for next segment
