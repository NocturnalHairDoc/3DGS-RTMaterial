...
/* Quick trace next 16 gaussians along the ray */
...
#pragma unroll
        for (int i = 0; i < PipelineParameters::MaxNumHitPerTrace; i++) {
            ...
            if ((rayHit.particleId != RayHit::InvalidParticleId) && (rayTransmittance > params.minTransmittance)) {
                ...
                rayData.hitCount += hitWeight > 0.f ? 1u : 0u;
#endif
            }
        }
    }
...
static __device__ __inline__ void intersectVolumetricGS() {
    float hitDistance;
...
static __device__ __inline__ void anyhitSortVolumetricGS() {
