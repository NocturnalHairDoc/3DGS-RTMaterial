...
def _find_cuda_home() -> str | None:
    ...
    for candidate in [
        os.path.join(program_files, "NVIDIA GPU Computing Toolkit", "CUDA", "v12.8"),
        os.path.join(program_files, "NVIDIA GPU Computing Toolkit", "CUDA", "v12.6"),
    ]:
...
Step 1: Install CUDA Toolkit 12.8 (including nvcc)
    https://developer.nvidia.com/cuda-downloads

Step 2: The setup script downloads the OptiX development headers
    Or set OPTIX_HOME to an installed NVIDIA OptiX SDK root.

Step 3: Install slangtorch
    pip install slangtorch

Step 4: Install the downloaded threedgrut Python package
    cd v_windows\\vendor\\3dgrut
    pip install -e . --no-build-isolation

Step 5: Install Visual Studio 2022 Build Tools
    Select Desktop development with C++ and Windows 10/11 SDK

Step 6: Compile the plugin from a Developer PowerShell
