...
from windows_bootstrap import configure_windows_runtime, windows_font_candidates
...
class CONFIG:
    r = 2   # scale ratio (smaller r = larger window: r=1 -> 2160x1200, r=2 -> 1080x600)
    ...
    # Right panel: larger when window is larger
    control_width = int(300 * (2 / r))
    control_height = int(550 * (2 / r))
    font_size = min(28, max(16, int(18 * (2 / r))))  # font size for control panel (capped 16–28)
...
class GaussianSplattingGUI:
    ...
    def register_dpg(self):
        ### register larger font for control panel (scale with window)
        _control_font = None
        font_paths = windows_font_candidates()
        for fp in font_paths:
            if os.path.isfile(fp):
                try:
                    with dpg.font_registry():
                        _control_font = dpg.add_font(fp, self.opt.font_size)
                    break
                except Exception:
                    pass
        self._control_font = _control_font

        ### register texture
        with dpg.texture_registry(show=False):
            dpg.add_raw_texture(self.width, self.height, self.render_buffer, format=dpg.mvFormat_Float_rgb, tag="_texture")

        cw = getattr(self.opt, 'control_width', 300)
        ch = getattr(self.opt, 'control_height', 550)
        ### register window
        with dpg.window(tag="_primary_window", width=self.window_width + cw, height=self.window_height):
        ...
        # control window (size scales with --scale)
        cw = getattr(self.opt, 'control_width', 300)
        ch = getattr(self.opt, 'control_height', 550)
        with dpg.window(label="Control", tag="_control_window", width=cw, height=ch, pos=[self.window_width+10, 0]):
        ...
        if self._control_font is not None:
            dpg.bind_item_font("_control_window", self._control_font)

        cw = getattr(self.opt, 'control_width', 300)
        dpg.create_viewport(title="Gaussian-Splatting-Viewer", width=self.window_width + cw + 20, height=self.window_height, resizable=False)
    ...
    def pca(self, X, n_components=3):
        ...
        eigenvalues_complex, eigenvectors_complex = torch.linalg.eig(covariance_matrix)
        # For real symmetric matrix, eigenvalues are real; take real part (or magnitude)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--scale', type=float, default=2.0, help='Window scale: smaller = larger window (e.g. 1.0 -> 2160x1200, 2.0 -> 1080x600)')
    ...
    opt.control_width = int(300 * (2 / opt.r))
    opt.control_height = int(550 * (2 / opt.r))
