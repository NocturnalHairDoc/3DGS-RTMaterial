...
import torch
from utils.knn_utils import knn_points
import numpy as np
...
class FeatureGaussianModel:
    ...
    def segment(self, mask=None):
        """Non-destructive segment: only update _mask, do not prune point cloud."""
        ...
        self.old_mask.append(self._mask.clone())

        self.segment_times += 1
        self._mask[to_mark] = self.segment_times + 1
    ...
    @torch.no_grad()
    def _get_feature_smooth_map(self, K):
        if self.feature_smooth_map is not None and self.feature_smooth_map["K"] == K:
        ...
        if self.knn_cache_dir is not None:
            cache_path = os.path.join(
                self.knn_cache_dir,
                f"feature_knn_k{K}_n{self.get_xyz.shape[0]}.pt",
            )
        nearest_k_idx = None
        if cache_path is not None and os.path.isfile(cache_path):
            try:
                cached = torch.load(cache_path, map_location="cpu", weights_only=True)
                if cached.shape == (self.get_xyz.shape[0], K):
                    print(f"Loading feature KNN cache: {cache_path}")
                    nearest_k_idx = cached.to(device=self.get_xyz.device, dtype=torch.long)
            except (OSError, RuntimeError, TypeError):
                nearest_k_idx = None

        if nearest_k_idx is None:
            xyz = self.get_xyz
            nearest_k_idx = knn_points(
                xyz.unsqueeze(0),
                xyz.unsqueeze(0),
                K=K,
            ).idx.squeeze(0)
            if cache_path is not None:
                ...
                print(f"Saving reusable feature KNN cache: {cache_path}")
                torch.save(nearest_k_idx.detach().to(device="cpu", dtype=torch.int32), cache_tmp)
    ...
    def smooth_point_features(self, K = 16, smoothed_dim = 24):
        nearest_k_idx = self._get_feature_smooth_map(K)
    ...
    def get_smoothed_point_features(self, K = 16, dropout = 0.5):
        ...
        if dropout > 0 and dropout < 1:
            ...
            ret = normed_features[nearest_k_idx, :].mean(dim = 1)
    ...
    def load_ply_from_3dgs(self, path):
        self.knn_cache_dir = os.path.dirname(path)
        plydata = PlyData.read(path)
