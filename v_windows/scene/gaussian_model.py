...
class GaussianModel:
    ...
    def prune_points(self, mask):
        ...
        # _mask is intentionally not optimized during RGB training, but it
        # still must follow the same point topology as every Gaussian tensor.
    ...
    def segment(self, mask=None):
        ...
        mask: True for points to assign to the new segment. Only unsegmented (_mask==1) points are marked."""
        assert mask is not None
        mask = mask.squeeze()
        assert mask.shape[0] == self._xyz.shape[0], f"mask {mask.shape[0]} != xyz {self._xyz.shape[0]}"
        if torch.count_nonzero(mask) == 0:
            mask = ~mask
            print("Seems like the mask is empty, segmenting the whole point cloud. Please run seg.py first.")

        # Only mark points that are currently unsegmented (_mask==1)
